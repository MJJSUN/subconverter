...
void xhttpDownloadToClash(YAML::Node node, const XhttpDownloadSettings &settings)
{
  ...
  if (!settings.XPaddingPlacement.empty())
    node["x-padding-placement"] = settings.XPaddingPlacement;
  if (!settings.XPaddingMethod.empty())
...
void xhttpConfigToClash(YAML::Node node, const Proxy &proxy)
{
  ...
  if (!proxy.Xhttp.XPaddingPlacement.empty())
    node["x-padding-placement"] = proxy.Xhttp.XPaddingPlacement;
  if (!proxy.Xhttp.XPaddingMethod.empty())
    node["x-padding-method"] = proxy.Xhttp.XPaddingMethod;
...
rapidjson::Value xhttpDownloadToJson(const XhttpDownloadSettings &settings, rapidjson::MemoryPoolAllocator<> &allocator)
{
  ...
  if (!settings.XPaddingPlacement.empty())
    xhttpExtra.AddMember("xPaddingPlacement", rapidjson::Value(settings.XPaddingPlacement.c_str(), allocator), allocator);
  if (!settings.XPaddingMethod.empty())
    xhttpExtra.AddMember("xPaddingMethod", rapidjson::Value(settings.XPaddingMethod.c_str(), allocator), allocator);
...
std::string buildVlessXhttpExtra(const Proxy &proxy)
{
  ...
  if (!proxy.Xhttp.XPaddingPlacement.empty())
    document.AddMember("xPaddingPlacement", rapidjson::Value(proxy.Xhttp.XPaddingPlacement.c_str(), allocator), allocator);
  if (!proxy.Xhttp.XPaddingMethod.empty())
    document.AddMember("xPaddingMethod", rapidjson::Value(proxy.Xhttp.XPaddingMethod.c_str(), allocator), allocator);
      ...
      else if (ssr)
      {
        if (std::find(ssr_ciphers.begin(), ssr_ciphers.end(), method) != ssr_ciphers.end() &&
          ...
          proxyStr = "ssr://" + urlSafeBase64Encode(
                                    ...
                                    urlSafeBase64Encode(password) + "/?group=" + urlSafeBase64Encode(x.Group) + "&remarks=" + urlSafeBase64Encode(remark));
      }
      else
        continue;
      break;
    case ProxyType::ShadowsocksR:
      if (ssr)
      {
        proxyStr = "ssr://" + urlSafeBase64Encode(
                                  ...
                                  urlSafeBase64Encode(password) + "/?group=" + urlSafeBase64Encode(x.Group) + "&remarks=" + urlSafeBase64Encode(remark) + "&obfsparam=" + urlSafeBase64Encode(obfsparam) + "&protoparam=" +
    ...
    case ProxyType::VLESS:
    {
      ...
      if (!transproto.empty())
      {
        ...
        case "grpc"_hash:
          ...
          params += "&mode=" + ((mode.empty() || mode == "gun") ? "multi" : mode);
          break;
        case "quic"_hash:
          params += "&headerType=" + fake_type;
                 ...
                 const ProxyGroupConfigs &extra_proxy_group, extra_settings &ext)
{
  ...
  for (Proxy &x : nodes)
  {
    ...
    std::string &hostname = x.Hostname, &method = x.EncryptMethod, &password = x.Password, &id = x.UserId, &transproto = x.TransferProtocol, &host = x.Host, &path = x.Path, &edge = x.Edge, &protocol = x.Protocol,
                &protoparam = x.ProtocolParam, &obfs = x.OBFS, &obfsparam = x.OBFSParam, &plugin = x.Plugin, &pluginopts = x.PluginOption, &username = x.Username;
    std::string port = std::to_string(x.Port);
    bool &tlssecure = x.TLSSecure;
    tribool scv;
    ...
    case ProxyType::ShadowsocksR:
      if (ext.nodelist)
      {
        proxyStr = "ssr://" + urlSafeBase64Encode(
                                  hostname + ":" + port + ":" + protocol + ":" + method + ":" + obfs + ":" +
                                  urlSafeBase64Encode(password) + "/?group=" + urlSafeBase64Encode(x.Group) + "&remarks=" + urlSafeBase64Encode(x.Remark) + "&obfsparam=" + urlSafeBase64Encode(obfsparam) + "&protoparam=" +
                                  urlSafeBase64Encode(protoparam));
      }
      else
      {
                  ...
                  const ProxyGroupConfigs &extra_proxy_group, extra_settings &ext)
{
  ...
  for (Proxy &x : nodes)
  {
    ...
    std::string &hostname = x.Hostname, &method = x.EncryptMethod, &id = x.UserId, &transproto = x.TransferProtocol,
                &host = x.Host, &path = x.Path, &password = x.Password, &plugin = x.Plugin, &pluginopts = x.PluginOption, &protocol = x.Protocol, &protoparam = x.ProtocolParam, &obfs = x.OBFS, &obfsparam = x.OBFSParam, &username = x.Username;
    std::string port = std::to_string(x.Port);
    bool &tlssecure = x.TLSSecure;
    ...
    case ProxyType::VLESS:
      if (flow != "xtls-rprx-vision")
      {
        if (transproto == "ws")
        {
          proxy = "VLESS," + hostname + "," + port + ",\"" + id + "\"" +
                  ...
                  ",udp=" + (udp.get() ? "true" : "false") + ",over-tls=" + (tlssecure ? "true" : "false") + ",sni=" + sni;
        }
        else
        {
          continue;
      ...
      else
      {
        proxy = "VLESS," + hostname + "," + port + ",\"" + id + "\",flow=" + flow + ",public-key=\"" + pk +
                "\",short-id=" + shortId + ",udp=" + (udp.get() ? "true" : "false") + ",over-tls=" + (tlssecure ? "true" : "false") + ",sni=" + sni;
      }
                    ...
                    const ProxyGroupConfigs &extra_proxy_group, extra_settings &ext)
{
  ...
  for (Proxy &x : nodes)
  {
    ...
    case ProxyType::WireGuard:
    {
      ...
      proxy.AddMember("inet4_bind_address", rapidjson::Value(x.SelfIP.c_str(), allocator), allocator);
