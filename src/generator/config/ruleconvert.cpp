...
void rulesetToSingBox(rapidjson::Document &base_rule, std::vector<RulesetContent> &ruleset_content_array, bool overwrite_original_rules)
{
  ...
  if (!overwrite_original_rules)
  {
    if (base_rule.HasMember("route"))
    {
      ...
      // 移除空 rule_set
      if (base_rule["route"]["rule_set"].Empty())
        base_rule["route"].RemoveMember("rule_set");
    }
  }
  ...
  // Add rule_set if there are any
  if (rule_sets.Size() > 0)
  {
    base_rule["route"].AddMember("rule_set", rule_sets, allocator);
  }
}
